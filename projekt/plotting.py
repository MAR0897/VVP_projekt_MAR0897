...
def plot(planets: List[Planet], print_mode: int, iter_n: int) -> None:
    ...
    Decides whether to generate an image or a video.
        For print_mode 1 or 2 selects plot_image,
...
def plot_image(planets: List[Planet], iter_n: int, print_mode: int) -> None:
    ...
    for i, planet in enumerate(planets):
        if print_mode == 1:
            ...
            ax.plot(planet.trajectory[0,:iter_n], planet.trajectory[1,:iter_n], label=planet.name, color=color_name)
        if print_mode == 2:
    ...
    if print_mode==1: 
        for i, planet in enumerate(planets):
            color_name = COLORS[i % len(COLORS)]
            ax.plot(planet.trajectory[0,iter_n-1], planet.trajectory[1,iter_n-1], label=planet.name, marker="o", color=color_name)
    ax.axis('equal')      
    plt.show()
...
def plot_video(planets: List[Planet], iter_n: int) -> None:
    ...
    def animate(frame):
        ...
        plt.legend()
    anim = FuncAnimation(fig=fig, func=animate, frames=int(iter_n/10), interval=10000/int(iter_n/10))
    anim.save("animation2.gif", writer="PillowWriter")